...
check_python310() {
    ...
    local -a python_candidates=("python3.13" "python310")
    ...
    for cmd in "${python_candidates[@]}"; do
        if command -v "$cmd" &>/dev/null; then
            ...
            if [[ "$version" == "3.13" ]]; then
                PYTHON_CMD="$cmd"
                print_success "Python 3.13 found: $("$cmd" --version)"
                return 0
            fi
        fi
    done
    ...
    if command -v python3 &>/dev/null; then
        ...
        if [[ "$version" == "3.13" ]]; then
            PYTHON_CMD="python3"
            print_success "Python 3.13 found: $(python3 --version)"
            return 0
        fi
    fi

    print_error "Python 3.13 is required but not found!"
    printf "\n${YELLOW}Install Python 3.13:${NC}\n"
    ...
    printf "    ${CYAN}sudo apt install python3.13 python3.13-venv${NC}\n"
    printf "\n  ${GREEN}Fedora:${NC}\n"
    printf "    ${CYAN}sudo dnf install python3.13${NC}\n"
    die "Please install Python 3.13 and try again"
}
...
select_rocm_version() {
    ...
    case "$gpu_gen" in
        ...
        rdna3)
            ...
            printf "  → ROCm 6.3 or 7.2 both work\n\n"
            ;;
        *)
            printf "${YELLOW}GPU generation unknown${NC}\n"
            printf "  → Try ROCm 7.2 (latest stable)\n\n"
            ;;
    esac

    printf "Available options:\n"
    printf "  ${GREEN}1)${NC} ROCm 6.3 (nightly) - Best compatibility for RX 6000/7000\n"
    printf "  ${GREEN}2)${NC} ROCm 7.2 (stable)  - Latest stable release\n"
    ...
    case "$choice" in
        ...
        2)
            ROCM_VERSION="7.2"
            export TORCH_COMMAND="uv pip install $force_reinstall torch torchvision --index-url https://download.pytorch.org/whl/rocm7.2"
            ;;
        3)
...
setup_venv() {
    ...
    print_info "Setting up Python 3.13 virtual environment..."

    cd "${install_dir}/${clone_dir}" || die "Cannot cd to ${install_dir}/${clone_dir}"

    if [[ ! -d "${venv_dir}" ]]; then
        print_info "Creating new venv..."
        uv venv "${venv_dir}" --python=python3.13 || die "Failed to create venv with uv"
        first_launch=1
...
main() {
    ...
    while true; do
        if [[ -n "${ACCELERATE:-}" ]] && [[ "${ACCELERATE}" == "True" ]] && command -v accelerate &>/dev/null; then
            ...
            accelerate launch --num_cpu_threads_per_process=6 "${LAUNCH_SCRIPT}" --force-fp16 --reserve-vram 11 --novram "$@" "${EXTRA_ARGS[@]}"
        else
            "${PYTHON_CMD}" -u "${LAUNCH_SCRIPT}" --force-fp16 --reserve-vram 11 --novram "$@" "${EXTRA_ARGS[@]}"
        fi
